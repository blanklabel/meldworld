...
async fn two_players_form_a_lobby_and_dive_together() {
    ...
    host.send("lobby.create", json!({ "party": ["resonant", "psyker", "explorer", "explorer"] })).await;
    ...
    guest.send("lobby.join", json!({ "code": code, "party": ["psyker", "psyker", "resonant", "explorer"] })).await;
