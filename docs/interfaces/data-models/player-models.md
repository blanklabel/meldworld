...
| unlocked_classes | array of string (enum: explorer, dragoon, sage, ranger, alchemist_knight, bard) | Yes | No | `["explorer"]` | v0.1 | No | Character classes available to the account. `explorer` is always present; every other class is added by acquiring the matching `ClassEmblem`. The launch set is a content placeholder and may be extended (CANON D9). |
...
- Invariant: `unlocked_classes` always contains `explorer` and contains no duplicates.
...
| character_class | string (enum: dragoon, sage, ranger, alchemist_knight, bard) | Yes | No | — | v0.1 | No | The character class this emblem unlocks. `explorer` never appears — it is the default class, not a drop. |
