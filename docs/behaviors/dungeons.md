...
- **The boss** (a `mob:<boss-key>:hostile` cell whose sprite is an authored named boss, e.g. `hollowbishop`) renders as an **animated, camera-facing `CharSprite`** — its 8-direction idle/walk frames driven by `animate_chars`, looming larger than a hero — not the single frozen billboard the fallback creatures use. Regular single-art creatures keep the billboard. When the authored sprite is one of the ten named bosses ([`meld_proto::bosses`](../../shared/meld-proto/src/bosses.rs)) the cell also carries the `:boss:<key>` token the overworld uses, so it wears its **title** on a name plate down here too; bespoke art that is not a named boss stays untitled rather than being labelled a guess.
