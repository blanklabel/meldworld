...
- **Flee has a toll (economy.md S1).** The run **continues** — you are not dead — but fleeing spills part of your haul: on a successful flee each fleeing player forfeits `flee_chit_loss_fraction` of their un-banked chits **[TUNABLE]** (default **0.5**), and every non-permanent item they carry (Party Inventory material, **every hero's pouch**, and red-chest looted gear) is independently dropped with probability `flee_item_drop_chance` **[TUNABLE]** (default **0.25**). Both containers are rolled: an exempt pouch would make "stuff the potions onto the heroes" a way to flee for free. Insured (blue) equipped gear is owned, not carried, and is never at risk. This is what makes fleeing a real decision rather than a free reset; the toll is applied server-side and mirrored to the client via `run.backpack_update` (removals for the **inventory half only** + negative `chits_delta`), a fresh `run.pouches` when any pouch spilled, and a fresh `run.gear` snapshot — with the `battle.ended`/`fled` message reporting everything dropped from both.
...
2. XP accumulates against `xp_to_next(L)`; the curve is stated in **fights**, not points: level `L` costs `[runs] fights_per_level_base` same-level encounters plus one more every `fights_per_level_ramp` levels, and one more every `fights_per_level_ramp_late` levels once past `fights_per_level_knee`. `xp_to_next(L)` multiplies that by what a same-level encounter actually pays (`meld_run::xp_to_next`), so retuning creature XP retunes the ladder with it. At shipped values (base 2, ramp 5, knee 100, late ramp 15): L10 costs 22 at-level fights, L20 65, L30 128, L100 21 and L255 31 — the deep ladder **plateaus** rather than flattening. On crossing the threshold the hero levels (multiple levels per battle possible), up to `[runs] max_hero_level` = 255 **[TUNABLE]**.
