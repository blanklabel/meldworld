...
| D6 | Durability loss on death unspecified | −`durability_loss_per_fall` **points** of max durability **per HERO DEATH**, floor 0, charged on that hero's own equipped **insured** gear only. Flat points, not a fraction: a piece's own `max_durability` is therefore the number of deaths it has left in it (`ceil(durability / loss)`), which is what lets one drop be better MADE than another — durability is rolled per piece (`gear_base_durability` ± `gear_durability_jitter`, ~3 deaths on average) and the `masterwork` affix buys more. A **fall**, not a wipe: a hero the party revives or carries home still pays, and a hero raised and killed again in one fight pays twice — so a party wipe is not a special case but every hero falling at once. Standard gear equipped at a wipe is destroyed outright and ephemeral burns on every way home; durability is what *insured* gear pays instead. Gear at 0 max durability is unequippable until repaired and contributes nothing. **[TUNABLE]** |
...
- Run Level cap: `[runs] max_hero_level` = 255 **[TUNABLE]**. The curve is stated in **fights**, not points: level `L` costs `fights_per_level_base` same-level encounters plus one more every `fights_per_level_ramp` levels — and, past `fights_per_level_knee`, one more every `fights_per_level_ramp_late` levels instead. `xp_to_next(L)` multiplies that by what a same-level encounter actually pays (`meld_run::xp_to_next`). At shipped values (base 2, ramp 5, knee 100, late ramp 15) level 10 costs 22 at-level fights, level 20 costs 65, level 30 costs 128; level 100 costs 21 fights and level 255 costs 31 — the ladder **plateaus** past the knee but never flattens, because out-levelling the ground is the route D-AD7 leaves open to a party that cannot get the gear. The old closed form `80 × L^1.6` is retired.
