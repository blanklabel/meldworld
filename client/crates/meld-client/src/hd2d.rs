...
pub fn animate_chars(
    ...
    // Facing is chosen **relative to the camera** so the shown side always matches the
    // current view (orbiting reveals a character's other sides — the HD-2D illusion).
    // Project the camera's forward/right onto the ground plane for the screen basis.
