...
3. Guarantee (reachability): generation never produces an unreachable ring; for every `distance` there exists at least one traversable path from the Center Hub, subject only to Gatekeeper blockers (below).
